...
export default {
  ...
  methods: {
    ...
    search: function (name, start) {
      ...
      this.axios.post('/api/Services/CommerceService.asmx/SearchMemoes', {query: this.searchName, start: start}).then(res => {
        if (res) {
          console.log('查询', res)
    ...
    pinkSlip: function (id) {
      this.printDate = moment(new Date()).format('YYYY-MM-DD HH:mm:ss')
      this.isLoading = true
      this.axios.post('/api/Services/CommerceService.asmx/GetViewMemo', {memoid: id}).then(res => {
        if (res) {
          console.log('查询单个', res)
          this.pinkSlipFormVisible = true
          this.$nextTick(() => { // resetFields初始化到第一次打开dialog时里面的form表单里的值，所以先渲染form表单，后改变值，这样resetFields后未空表单
