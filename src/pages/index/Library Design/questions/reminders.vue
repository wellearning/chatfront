<!--
FileName: Library/questions/reminders.vue
Author: Ge Chen
Update Date: 2023/9/20
Function: Show reminder list and do all operations on the list.
-->
<template>
  <div>
    <div class="inPageTitle">
      <span class="inPageNav">Reminders for {{businessTypes[btypeId]}}</span>
...
export default {
  ...
  data: function () {
    return {
      ...
      typeName: ' Reminder Question List',
  ...
  methods: {
    ...
    search: function (name) {
      this.isLoading = true
      this.axios.post('/api/Services/BaseService.asmx/GetQuestionsByTypeQuery', {typeid: 2, query: name, btypeid: this.btypeId}).then(res => {
        if (res) {
          console.log('查询', res)
          this.list = res.data
          if (name !== null) {
