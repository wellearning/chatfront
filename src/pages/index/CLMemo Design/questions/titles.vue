...
export default {
  ...
  methods: {
    ...
    search: function (name) {
      this.isLoading = true
      this.axios.post('/api/Services/CommerceService.asmx/GetQuestionsByTypeQuery', {typeid: 1, query: name}).then(res => {
        if (res) {
          console.log('查询', res)
          this.list = res.data
          if (name !== null) {
    ...
    edit: function (obj) {
      ...
      if (!new RegExp(rule).test(obj.Description)) {
        ...
        this.isLoading = true
        this.axios.post('/api/Services/CommerceService.asmx/SaveQuestion', {question: JSON.stringify(obj)}).then(res => {
          if (res) {
            console.log('修改', res)
            this.$message({
              type: 'success',
