<template>
  <div>
    ...
    <div v-for="answer in applicationBlock.answers" :key="answer.QuestionID">
      ...
      <div>
        <i v-if="answer.StatusID === 1" class="el-icon-check"></i>
        <div class="answerMemo" v-if="answer.TypeID === 1">
          <div class="typeTitle">
...
export default {
  ...
  methods: {
    ...
    checkOver: function () {
      ...
      cb.Finished = true
      cb.answers.forEach(ca => {
        if (ca.TypeID === 1 || ca.TypeID === 2) return
        if (ca.StatusID === 2) return
        if (ca.StatusID === 0) {
          cb.Finished = false
        }
        if ((ca.TypeID === 3 || ca.TypeID === 4) && (ca.AnswerDesc === null || ca.AnswerDesc === undefined || ca.AnswerDesc.length === 0)) {
          cb.Finished = false
        }
        if (ca.TypeID === 5) {
          ca.partAnswers.forEach(pa => {
    ...
    changeVal: function (answer) {
      if (answer.InputType === 'array') answer.StatusID = 1
      if (answer.InputType === 'list') this.resetChildrenAnswer(answer)
      else if (answer.InputType !== 'computed') this.recomputeAttributes()
      // this.showNextQuestion(answer)
      this.setAnswerStatus(answer)
      this.checkOver()
    },
    setBlockAnswersStatus: function () {
      let cb = this.applicationBlock
      cb.answers.forEach(answer => {
        this.setAnswerStatus(answer)
      })
    },
    setAnswerStatus: function (answer) {
      if (answer.TypeID === 3) {
        if (answer.InputType === 'array') answer.StatusID = 1
        else if (answer.InputType === 'list') answer.StatusID = 1
        else {
          if (answer.AnswerDesc === null || answer.AnswerDesc.length === 0) answer.StatusID = 0
          else answer.StatusID = 1
        }
      } else if (answer.TypeID === 4) {
        if (answer.AnswerDesc === null || answer.AnswerDesc.length === 0) answer.StatusID = 0
        else answer.StatusID = 1
      } else if (answer.TypeID === 5) {
        answer.partAnswers.forEach(pa => {
          if (pa.IsFillin && pa.Part.length === 0) {
            answer.StatusID = 0
          } else answer.StatusID = 1
        })
      } else if (answer.TypeID === 6) {
        if (answer.checkvalue === 0) answer.StatusID = 0
        else answer.StatusID = 1
      } else if (answer.TypeID === 7) {
        if (answer.checklist.length === 0) answer.StatusID = 0
        else answer.StatusID = 1
      }
    }
